...
public class DetectViolationsOnCorpus 
{
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        File CORPUS = new File("../DatasetCreation/CORPUS/DEPLOYonUbuntu/DLV2SPARQL");
        ...
        while(temp.isEmpty()==false)
        {
            ...
            for(int i=1;i<temp.size();i++)
            {
                ...
                nmin = nmin.substring(nmin.indexOf("StateOfAffair_")+"StateOfAffair_".length(), nmin.indexOf("_",nmin.indexOf("StateOfAffair_")+"StateOfAffair_".length())).trim();
                nmin = nmin.substring(nmin.indexOf("-")+1,nmin.length()).trim();
                int smin = Integer.parseInt(nmin);
                String ni = temp.get(i).getName();
                ni = ni.substring(ni.indexOf("StateOfAffair_")+"StateOfAffair_".length(), ni.indexOf("_",ni.indexOf("StateOfAffair_")+"StateOfAffair_".length()));
                ni = ni.substring(ni.indexOf("-")+1,ni.length()).trim();
